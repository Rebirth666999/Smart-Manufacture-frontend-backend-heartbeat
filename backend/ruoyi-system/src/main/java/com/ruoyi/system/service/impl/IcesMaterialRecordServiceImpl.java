...
public class IcesMaterialRecordServiceImpl implements IIcesMaterialRecordService {
    ...
    private LambdaQueryWrapper<IcesMaterialRecord> buildQueryWrapper(IcesMaterialRecordBo bo) {
        ...
        lqw.eq(StringUtils.isNotBlank(bo.getMrCode()), IcesMaterialRecord::getMrCode, bo.getMrCode());
        lqw.eq(StringUtils.isNotBlank(bo.getMaCode()), IcesMaterialRecord::getMaCode, bo.getMaCode());
        lqw.eq(StringUtils.isNotBlank(bo.getStCode()), IcesMaterialRecord::getStCode, bo.getStCode());
        lqw.eq(StringUtils.isNotBlank(bo.getMrType()), IcesMaterialRecord::getMrType, bo.getMrType());
        lqw.eq(StringUtils.isNotBlank(bo.getMrStat()), IcesMaterialRecord::getMrStat, bo.getMrStat());
        lqw.eq(bo.getMrEst() != null, IcesMaterialRecord::getMrEst, bo.getMrEst());
        lqw.eq(bo.getMrReal() != null, IcesMaterialRecord::getMrReal, bo.getMrReal());
        lqw.eq(bo.getMrDelete() != null, IcesMaterialRecord::getMrDelete, bo.getMrDelete());
        lqw.eq(StringUtils.isNotBlank(bo.getMrDesc()), IcesMaterialRecord::getMrDesc, bo.getMrDesc());
        return lqw;
    }
    ...
    public Boolean insertByBo(IcesMaterialRecordBo bo) {
        bo.setMrCode(codeService.insertByType("MaterialRecord"));
