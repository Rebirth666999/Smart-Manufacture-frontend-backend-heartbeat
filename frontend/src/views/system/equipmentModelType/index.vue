<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="名称" prop="emtName">
        <el-input
          v-model="queryParams.emtName"
          placeholder="请输入名称"
          clearable
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:equipmentModelType:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:equipmentModelType:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:equipmentModelType:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="名称" prop="emtName">
          <el-input v-model="form.emtName" placeholder="请输入名称" />
        </el-form-item>
        <el-form-item label="描述" prop="emtDesc">
          <el-input v-model="form.emtDesc" type="textarea" placeholder="请输入内容" />
        </el-form-item>
      </el-form>
...
<script>
import { listEquipmentModelType, getEquipmentModelType, delEquipmentModelType, addEquipmentModelType, updateEquipmentModelType } from "@/api/system/equipmentModelType";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        emtId: [
          { required: true, message: "模型类型ID不能为空", trigger: "blur" }
        ],
        emtName: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listEquipmentModelType(this.queryParams).then(response => {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.emtId)
    ...
    handleUpdate(row) {
      this.loading = true;
      this.reset();
      const emtId = row.emtId || this.ids
      getEquipmentModelType(emtId).then(response => {
        this.loading = false;
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.emtId != null) {
            updateEquipmentModelType(this.form).then(response => {
            ...
            addEquipmentModelType(this.form).then(response => {
